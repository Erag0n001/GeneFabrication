...
public class GeneUI : Window
{
	...
	public GeneUIMode CurrentGeneUIMode;
	public GeneUI(BillStack stack, GeneUIMode geneUIMode)
	{
		this.stack = stack;
		draggable = true;
		this.CurrentGeneUIMode = geneUIMode;
	}
	public override void DoWindowContents(Rect rect)
	{
		...
		float height = 6f + genes.Count * HeightOfRow;
	...
	private void DrawCustomRow(Rect rect, GeneDef thing, int index)
	{
		...
		if (index % 2 == 0)
		{
			if(CurrentGeneUIMode == GeneUIMode.Normal && (recipe.researchPrerequisite == null || recipe.researchPrerequisite.IsFinished))
				Widgets.DrawLightHighlight(highLightRect); //Highlight half the cells
			else if(CurrentGeneUIMode == GeneUIMode.Blacklist)
				Widgets.DrawLightHighlight(highLightRect); //Highlight half the cells
		}
		...
		if(CurrentGeneUIMode == GeneUIMode.Blacklist && ModConfigs.BlacklistedGenes.Any(G => G == thing.defName))
		{
			Widgets.DrawStrongHighlight(highLightRect, Widgets.InactiveColor);
		}
		// Actual code
		...
		void OnClick()
		{
			...
			{
				if (CurrentGeneUIMode == GeneUIMode.Normal &&
				...
				{
					if (ModConfigs.BlacklistedGenes.Any(G => G == thing.defName))
						ModConfigs.BlacklistedGenes.Remove(thing.defName);
					else
						ModConfigs.BlacklistedGenes.Add(thing.defName);
				}
			}
		}
		void ShowHovering()
		{
			if (Mouse.IsOver(highLightRect)) //Hover effect
			{
				...
				if(CurrentGeneUIMode == GeneUIMode.Blacklist && CurrentGeneUIMode == GeneUIMode.Blacklist && ModConfigs.BlacklistedGenes.Any(G => G == thing.defName))
				{
	...
	private List<GeneDef> GenesInSearch()
	{
		...
		{
			if (CurrentGeneUIMode == GeneUIMode.Normal && ModConfigs.BlacklistedGenes.Any(G => G.ToLower() == gene.defName.ToLower()))
				continue;
