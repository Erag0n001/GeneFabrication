...
namespace GeneFarbication
{
	public static class RecipeDefGenerator
	{
		...
		{
			if (ThingDef.Named("GeneFab_GeneBench") is not ThingDef def)
				return;
		...
		public static List<IngredientCount> CalcIngredients(GeneDef gene)
		{
			...
			neutroamine.SetBaseCount((int)(Mathf.Clamp(
				...
				50, 400) * ModConfigsUI.config.neutroamineMultiplier));
			...
			if (gene.biostatArc != 0)
			{
				...
				architeCapsule.SetBaseCount((int)(Mathf.Abs(gene.biostatArc) * 3 * ModConfigsUI.config.architeMultiplier));
		...
		public static int CalcWorkAmount(GeneDef gene)
		{
			return (int)(Mathf.Clamp(24000
				...
				20000, 50000) * ModConfigsUI.config.workMultiplier);
		}
